...
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->setCellValue('A1', 'Financial Year')
	...
	foreach($periods as $period) {
		foreach($countries as $country) {
			...
			for($i = 1; $i <= $endDays; ++$i) {
				...
				$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->fromArray($dataArray, NULL, 'A'.$row++);
			}
		}
	}
}
...
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle('A1:F1')->getFont()->setBold(true);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle('A1:F1')->getAlignment()->setWrapText(TRUE);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getColumnDimension('C')->setWidth(12.5);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getColumnDimension('D')->setWidth(10.5);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle('D2:D'.$row)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_YYYYMMDD2);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle('E2:F'.$row)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getColumnDimension('F')->setWidth(14);
...
$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->setAutoFilter($obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->calculateWorksheetDimension());

// Set active filters
$autoFilter = $obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getAutoFilter();
...
foreach ($obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getRowIterator() as $row) {
	if ($obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getRowDimension($row->getRowIndex())->getVisible()) {
		echo '    Row number - ' , $row->getRowIndex() , ' ';
		echo $obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell('C'.$row->getRowIndex())->getValue(), ' ';
		echo $obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell('D'.$row->getRowIndex())->getFormattedValue(), ' ';
		echo EOL;
	}
}
