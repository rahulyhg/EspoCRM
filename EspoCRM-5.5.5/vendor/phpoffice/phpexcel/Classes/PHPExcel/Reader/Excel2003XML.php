 ...
 * @copyright  Copyright (c) 2006 - 2014 \PhpOffice\PhpSpreadsheet\Spreadsheet (http://www.codeplex.com/\PhpOffice\PhpSpreadsheet\Spreadsheet)
 */
class \PhpOffice\PhpSpreadsheet\Reader\Xml extends \PhpOffice\PhpSpreadsheet\Reader\BaseReader implements \PhpOffice\PhpSpreadsheet\Reader\IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new \PhpOffice\PhpSpreadsheet\Reader\Xml
	 */
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	public function load($pFilename)
	{
        ...
        $obj\PhpOffice\PhpSpreadsheet\Spreadsheet->removeSheetByIndex(0);
	...
	public function loadIntoExisting($pFilename, \PhpOffice\PhpSpreadsheet\Spreadsheet $obj\PhpOffice\PhpSpreadsheet\Spreadsheet)
	{
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','\PhpOffice\PhpSpreadsheet\Reader\Xml::_hex2str',$propertyName);
...
//						echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
						$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getColumnDimension($columnID)->setWidth($columnWidth / 5.4);
					}
					++$columnID;
				}
			}
...
//
							$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rowID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue),$type);
							if ($hasCalculatedValue) {
//								echo 'Formula result is '.$cellValue.'<br />';
								$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
							}
...
//								echo '<br />';
								if (!$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->cellExists($columnID.$rowID)) {
									$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
								}
								$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->_styles[$style]);
							}
						}
						++$columnID;
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
							$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getRowDimension($rowID)->setRowHeight($rowHeight);
						}
					}
