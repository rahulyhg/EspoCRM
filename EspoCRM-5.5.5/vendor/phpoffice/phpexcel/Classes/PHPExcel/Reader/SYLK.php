 ...
 * @copyright  Copyright (c) 2006 - 2014 \PhpOffice\PhpSpreadsheet\Spreadsheet (http://www.codeplex.com/\PhpOffice\PhpSpreadsheet\Spreadsheet)
 */
class \PhpOffice\PhpSpreadsheet\Reader\Slk extends \PhpOffice\PhpSpreadsheet\Reader\BaseReader implements \PhpOffice\PhpSpreadsheet\Reader\IReader
{
	/**
	 * Input encoding
	 *
	...
	/**
	 * Create a new \PhpOffice\PhpSpreadsheet\Reader\Slk
	 */
	...
	public function loadIntoExisting($pFilename, \PhpOffice\PhpSpreadsheet\Spreadsheet $obj\PhpOffice\PhpSpreadsheet\Spreadsheet)
	{
		...
		// Create new \PhpOffice\PhpSpreadsheet\Spreadsheet
		while ($obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getSheetCount() <= $this->_sheetIndex) {
			$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->createSheet();
		}
		$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->setActiveSheetIndex( $this->_sheetIndex );
		...
		while (($rowData = fgets($fileHandle)) !== FALSE) {
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$formatArray['font']['bold'] = true;
														break;
											case 'T' :	$formatArray['borders']['top']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'B' :	$formatArray['borders']['bottom']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'L' :	$formatArray['borders']['left']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'R' :	$formatArray['borders']['right']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
										}
									}
									break;
			...
			//	Read cell value data
				...
				$cellData = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::_unwrapResult($cellData);

				// Set cell value
				$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnLetter.$row)->setValue(($hasCalculatedValue) ? $cellDataFormula : $cellData);
				if ($hasCalculatedValue) {
					$cellData = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::_unwrapResult($cellData);
					$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnLetter.$row)->setCalculatedValue($cellData);
				}
			//	Read cell formatting
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$styleData['font']['bold'] = true;
														break;
											case 'T' :	$styleData['borders']['top']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'B' :	$styleData['borders']['bottom']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'L' :	$styleData['borders']['left']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
											case 'R' :	$styleData['borders']['right']['style'] = \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN;
														break;
										}
									}
									break;
				...
				if (($formatStyle > '') && ($column > '') && ($row > '')) {
					...
					if (isset($this->_formats[$formatStyle])) {
						$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($this->_formats[$formatStyle]);
					}
				}
				if ((!empty($styleData)) && ($column > '') && ($row > '')) {
					$columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($column-1);
					$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($styleData);
				}
				if ($columnWidth > '') {
					if ($startCol == $endCol) {
						$startCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startCol-1);
						$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getColumnDimension($startCol)->setWidth($columnWidth);
					} else {
						$startCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($startCol-1);
