 ...
 * @copyright	Copyright (c) 2006 - 2014 \PhpOffice\PhpSpreadsheet\Spreadsheet (http://www.codeplex.com/\PhpOffice\PhpSpreadsheet\Spreadsheet)
 */
class \PhpOffice\PhpSpreadsheet\Reader\Ods extends \PhpOffice\PhpSpreadsheet\Reader\BaseReader implements \PhpOffice\PhpSpreadsheet\Reader\IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new \PhpOffice\PhpSpreadsheet\Reader\Ods
	 */
...
//		}
	...
	public function listWorksheetNames($pFilename)
	{
		...
		$xml = new XMLReader();
		$res = $xml->xml($this->securityScanFile('zip://'.realpath($pFilename).'#content.xml'), null, \PhpOffice\PhpSpreadsheet\Settings::getLibXmlLoaderOptions());
...
//									echo 'Cell has comment<br />';
									...
									$text = implode("\n",$textArray);
//									echo $text,'<br />';
									$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getComment( $columnID.$rowID )
//																	->setAuthor( $author )
...
//									echo 'Formula: ', $cellDataFormula, PHP_EOL;
									...
									foreach($temp as &$value) {
										...
										if ($tKey = !$tKey) {
											...
											$value = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::_translateSeparator(';',',',$value,$inBraces);
										}
									}
									unset($value);
									...
									for ($i = 0; $i < $colRepeats; ++$i) {
										...
										if ($type !== \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NULL) {
											for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
												$rID = $rowID + $rowAdjust;
												$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $dataValue),$type);
												if ($hasCalculatedValue) {
//													echo 'Forumla result is '.$dataValue.'<br />';
													$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rID)->setCalculatedValue($dataValue);
												}
												if ($formatting !== NULL) {
													$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle($columnID.$rID)->getNumberFormat()->setFormatCode($formatting);
												} else {
													$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getStyle($columnID.$rID)->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_GENERAL);
												}
												if ($hyperlink !== NULL) {
													$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->getCell($columnID.$rID)->getHyperlink()->setUrl($hyperlink);
												}
											}
										}
									}
								...
								if ((isset($cellDataTableAttributes['number-columns-spanned'])) || (isset($cellDataTableAttributes['number-rows-spanned']))) {
									if (($type !== \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NULL) || (!$this->_readDataOnly)) {
										$columnTo = $columnID;
										...
										$obj\PhpOffice\PhpSpreadsheet\Spreadsheet->getActiveSheet()->mergeCells($cellRange);
									}
								}
