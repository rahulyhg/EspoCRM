...
/**
 * \PhpOffice\PhpSpreadsheet\ReferenceHelper (Singleton)
 *
 * @category   \PhpOffice\PhpSpreadsheet\Spreadsheet
 * @package	\PhpOffice\PhpSpreadsheet\Spreadsheet
 * @copyright  Copyright (c) 2006 - 2014 \PhpOffice\PhpSpreadsheet\Spreadsheet (http://www.codeplex.com/\PhpOffice\PhpSpreadsheet\Spreadsheet)
 */
class \PhpOffice\PhpSpreadsheet\ReferenceHelper
{
	/**	Constants				*/
	/**	Regular Expressions		*/
	...
	/**
	 * Create a new \PhpOffice\PhpSpreadsheet\ReferenceHelper
	 */
	...
	protected function _adjustPageBreaks(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aBreaks, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellReverseSort')) :
			uksort($aBreaks, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellSort'));
	...
	protected function _adjustHyperlinks($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aHyperlinkCollection, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellReverseSort')) :
			uksort($aHyperlinkCollection, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellSort'));
	...
	protected function _adjustDataValidations($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aDataValidationCollection, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellReverseSort')) :
			uksort($aDataValidationCollection, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellSort'));
	...
	protected function _adjustProtectedCells($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aProtectedCells, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellReverseSort')) :
			uksort($aProtectedCells, array('\PhpOffice\PhpSpreadsheet\ReferenceHelper','cellSort'));
			...
			for ($i = 1; $i <= $highestRow - 1; ++$i) {
				for ($j = $beforeColumnIndex - 1 + $pNumCols; $j <= $beforeColumnIndex - 2; ++$j) {
					...
					if ($pSheet->cellExists($coordinate)) {
						$pSheet->getCell($coordinate)->setValueExplicit('', \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NULL);
			...
			for ($i = $beforeColumnIndex - 1; $i <= \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($highestColumn) - 1; ++$i) {
				...
				if ($pSheet->cellExists($coordinate)) {
					...
					for ($j = $beforeRow; $j <= $beforeRow - 1 + $pNumRows; ++$j) {
						$pSheet->getCell(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i) . $j)->setXfIndex($xfIndex);
						if ($conditionalStyles) {
							$cloned = array();
							foreach ($conditionalStyles as $conditionalStyle) {
								$cloned[] = clone $conditionalStyle;
					...
					foreach($matches as $match) {
						...
						if ($match[3].':'.$match[4] !== $modified3.':'.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								...
								$toString .= $modified3.':'.$modified4;
								//	Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
								$column = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString(trim($match[3],'$')) + 100000;
								$row = 10000000;
								$cellIndex = $column.$row;
					...
					foreach($matches as $match) {
						...
						if ($match[3].$match[4] !== $modified3.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								...
								//	Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
								$column = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString(trim($column,'$')) + 100000;
